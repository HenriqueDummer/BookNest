...
const Book = () => {
  ...
  return (
    <section
      className="w-full overflow-auto bg-[radial-gradient(ellipse_at_bottom,_var(--tw-gradient-stops))]
        ...
        <div className="p-5">
          ...
          <div className="flex flex-wrap gap-3 px-5 lg:px-0 mt-8">
            {genres.map((genre) => {
              return (
                <p
                  key={genre}
                  className="bg-[#FF008A] px-4 py-2 rounded-full flex items-center gap-1"
                >
                  {genre}
                </p>
              );
          ...
          {isPublic && (
            <NavLink
              ...
              className="inline-flex items-center gap-3 mt-10 duration-200 bg-transparent cursor-pointer hover:bg-white/10 py-2 px-3 rounded-full"
            >
              <div
                style={{ backgroundImage: `url(${sharedBy.profileImg})` }}
                className="w-10 aspect-square bg-cover bg-center rounded-full"
              ></div>
